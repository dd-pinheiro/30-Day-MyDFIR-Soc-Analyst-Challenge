...
Um ataque de força bruta (Brute-Force Attack) é um tipo de ataque onde o adversário usa todo tipo de combinação de senha com o intuito de comprometer a conta de um usuário. Existem variações deste ataque, no entanto vou entrar em detalhes de apenas três delas.

### Brute Force Attack
Esse ataque utiliza um método "Try and Error" na tentativa de comprometer a conta de um usuário

### *Dictionary Attack*
Similar ao ataque de força bruta, no entanto utiliza uma "word list" que contém palavras comuns, frases e senhas encontradas em *credential dumps*.

### *Credential Stuffing*
Isso é onde o atacante obtém uma lista com vazamento de dados e tenta todo tipo de combinação de usuário e senha.
...
| :--- | :--- |
| **Hydra** | Ferramenta rápida e versátil para quebra de senhas via rede que suporta diversos protocolos (SSH, FTP, RDP, HTTP, etc.). |
...
- O que é um *Brute-force Attack* e suas variações
- Tipos de ferramentas utilizadas em Ataques de Brute-Force
- Como detectar e mitigar esses tipos de ataque em um ambiente empresarial
