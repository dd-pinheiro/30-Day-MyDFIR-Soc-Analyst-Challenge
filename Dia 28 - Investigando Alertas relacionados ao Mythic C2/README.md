...
Mas fica aqui um questionamento, ***Como posso investigar de uma forma mais detalhadas as atividades do Mythic?*** A forma mais comum é através de Telemetria de redes (Network Telemetry) e Investigação de Criação de Processos (Event ID 1); segue uma breve descrição:
...
## Investigação 
Pesquisando pelo nome do payload (`payload.exe`) na aba de *Discover* do Elastic, obtive os seguintes resultados: 
<img width="1200" height="514" alt="image" src="https://github.com/user-attachments/assets/f2ed247b-4940-4459-847d-43710fdfbedc" />

No entanto, meu objetivo é verificar as atividades relacionados aos Event ID's 3 e 1 na máquina, então utilizei meu dashboard criado para fácil visualização:
...
Para obter minhas respostas, realizei um *threat hunting* em relação às atividades.
...
1. Em "**Rules**" > **Detection Rules (SIEM)**, também localizada em *Security*; selecionei a regra de *Mythic C2 - Activities*.
...
Em um cenário real, eu poderia muito bem utilizar a URL do corpo do Ticket para analisar o alerta. Essa URL iria me direcionar para a aba de "**Alerts**" direto pra o Elastic SIEM, possibilitando assim que eu comece a realizar a triagem do Incidente.
